...
namespace beatmap {
    ...
    public:
        bool try_load(
            ...
            for ( int32_t i = 0; i < count; ++i ) {
                ...
                // +0xD0 was the old duration heuristic. On 2026.804.2 it is
                // inside OsuHitObject::lastInCombo and cannot classify object type.
                // A Slider has a real SliderPath at +0xE0. Validate that path via
                // SliderPath.calculatedPath (+0x20), a List<Vector2>.
                const auto path = process.read<uint64_t>( obj_ptr + off.slider_path_wrapper );
                const auto calc_path = path ? process.read<uint64_t>( path + off.path_ctrl_points_list ) : 0;
                const auto calc_items = calc_path ? process.read<uint64_t>( calc_path + off.list_items ) : 0;
                const auto calc_count = calc_path ? process.read<int32_t>( calc_path + off.list_size ) : 0;
                const bool is_slider = path && calc_items && calc_count >= 2 && calc_count <= 10000;

                const auto nested = process.read<uint64_t>( obj_ptr + off.hit_object_nested_objects );
                const auto nested_count = nested ? process.read<int32_t>( nested + off.list_size ) : 0;
                ...
                if ( is_slider ) {
                    ...
                    const int32_t repeats = process.read<int32_t>( obj_ptr + off.slider_repeat_count );
                    if ( repeats >= 0 && repeats < 100 )
                        obj.slider_repeat = repeats + 1;
                    read_slider_path( process, off, obj_ptr, obj, out.hr );
                }
                else if ( nested_count > 0 && nested_count < 1000 && is_center ) {
    ...
    private:
        ...
        static void read_slider_path(
            ...
            // SliderPath.calculatedPath is already a sampled curve. Encode it as
            // linear segments so the original Autobot slider evaluator consumes the
            // same absolute osu! coordinates without depending on PathType internals.
